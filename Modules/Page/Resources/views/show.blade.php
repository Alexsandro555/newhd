...
@section('sidebar')
  <h1><span>Датчики влажности</span></h1>
  <div class="information">
    <ul class="article__tab-action">
      @foreach($vlagas as $vlaga)
        <li>
          <a href="{{$vlaga->url_key.'.html'}}">
            @if(count($vlaga->files)>0)
              @foreach($vlaga->files[0]->config as $key=>$conf)
              ...
              @endforeach
            @endif
            {{$vlaga->short_title?$vlaga->short_title:$vlaga->title}}
          </a>
        </li>
      @endforeach
    </ul>
