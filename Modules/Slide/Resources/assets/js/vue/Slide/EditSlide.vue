                ...
                <v-flex>
                  <v-form ref="form" lazy-validation v-model="valid">
                    <template v-for="(field, num) in fields">
                      <form-builder :field="field" v-if="num!=='description'" :relations="relations" :num="num"
                    ...
                    <file-box url="/files/upload" :fileable-id="Number(form.id)" :type-files="typeFiles"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState('slides', ['items', 'fields', 'relations', 'typeFiles']),
      ...mapGetters('slides', {getItem: GLOBAL.GET_ITEM, getModel: 'getModel'}),
      form() {
        return _.pick(this.getItem(Number(this.id)), ['id', 'title', 'description', 'sort', 'active', 'url_key'])
      }
    },
    components: {
      formBuilder,
